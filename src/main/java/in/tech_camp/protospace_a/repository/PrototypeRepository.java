...
public interface PrototypeRepository {
    ...
    @Select("SELECT p.id, p.name, p.catchphrase, p.image, u.id AS user_id, u.username AS user_name "
            + "FROM prototypes p " + "JOIN users u ON p.user_id = u.id "
    ...
    @Select("SELECT p.*, u.id AS user_id, u.username AS user_name "
            + "FROM prototypes p " + "JOIN users u ON p.user_id = u.id "
            + "WHERE p.id = #{id}")
    @Results({@Result(property = "user.id", column = "user_id"),
            @Result(property = "user.username", column = "user_name")})
    ...
    @Results(value = {@Result(property = "id", column = "id"),
            ...
            @Result(property = "comments", column = "id", many = @Many(
                    select = "in.tech_camp.protospace_a.repository.CommentRepository.findByPrototypeId"))})
